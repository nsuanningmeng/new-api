...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		result, err := service.CompleteTopUpRecharge(verifyInfo.ServiceTradeNo, model.PaymentProviderEpay, verifyInfo.Type, "", "")
		if err != nil {
		...
		if !result.AlreadyCompleted {
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota=%d money=%.2f", verifyInfo.ServiceTradeNo, result.UserId, c.ClientIP(), result.Quota, result.Money))
			model.RecordTopupLog(result.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(int(result.Quota)), result.Money), c.ClientIP(), result.PaymentMethod, "epay")
		}
	} else {
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	if err := service.ManualCompleteTopUp(req.TradeNo, c.ClientIP()); err != nil {
		common.ApiError(c, err)
		return
	}
